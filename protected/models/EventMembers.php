...
//class EventMembers extends CActiveRecord
{
    ...
    const PAYMENT_FREE = 3;
    const PAYMENT_WAIT = 4;
...
//            'organizer' => [self::HAS_ONE, 'EventOrganizers', ['event_id' => 'user_id']],
		);
	}
...
//        return (new DateTime($this->b_date))->format('Y');
    ...
    {
        return [
            self::PAYMENT_SUCCESS => Yii::t('EventModule.event', 'Сплачено'),
            self::PAYMENT_ERROR  => Yii::t('EventModule.event', 'Помилка оплати'),
            self::PAYMENT_FREE  => Yii::t('EventModule.event', 'Реєстрація безкоштовна'),
            self::PAYMENT_WAIT  => Yii::t('EventModule.event', 'Очікування оплати'),
        ];
    }
    ...
    {
        $data = $this->getPaymentStatusList();

        return isset($data[$this->payment_status]) ? $data[$this->payment_status] : Yii::t('EventModule.event', '*unknown*');
    }
}
